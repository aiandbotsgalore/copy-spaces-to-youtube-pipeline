...
class PanelErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidUpdate(prevProps: ErrorBoundaryProps) {
    if (prevProps.activePanel !== this.props.activePanel && this.state.hasError) {
    ...
    if (this.state.hasError) {
      return (
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-950 text-slate-200">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Panel Error</h2>
          ...
          <div className="flex gap-3">
            <button
              onClick={this.reset}
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-xl shadow-lg transition-all cursor-pointer"
            >
              Try Again
            </button>
            <button
...
const DEFAULT_CONFIG: EnhancedConfig = {
  ...
  podcastTitle: "Logan Black's X-Space",
  podcastDescription: "Logan Black's X-Space Podcast Archive",
...
function buildInitialConfig(): EnhancedConfig {
  ...
  return {
    ...
    podcastTitle: (stored.podcastTitle && stored.podcastTitle !== 'My Spaces Archive' && stored.podcastTitle !== 'Twitter Spaces Archive') ? stored.podcastTitle : DEFAULT_CONFIG.podcastTitle,
    podcastDescription: (stored.podcastDescription && stored.podcastDescription !== 'Archive of Twitter Spaces') ? stored.podcastDescription : DEFAULT_CONFIG.podcastDescription,
    authorName: stored.authorName ? stored.authorName : DEFAULT_CONFIG.authorName,
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col md:flex-row font-sans">
      ...
      <main className="flex-1 min-w-0 flex flex-col h-screen overflow-hidden">
        <PanelErrorBoundary activePanel={activePanel}>
          <Suspense fallback={
