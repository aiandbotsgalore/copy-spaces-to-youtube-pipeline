    ...
    <div className="h-full overflow-y-auto p-6 md:p-12 max-w-4xl mx-auto w-full">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Live GitHub Actions workflow runs for <code className="text-sky-400 bg-slate-800 px-1 rounded text-xs">{config.ownerName}/{config.repoName}</code>
          </p>
        </div>
      ...
      {hasCredentials && !loaded && !loading && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-600 text-xs mt-1">Requires a valid GitHub token and repository</p>
        </div>
      )}
      ...
      {loaded && runs.length === 0 && !error && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-600 text-xs mt-1">Runs will appear here once you push files and the pipeline triggers.</p>
        </div>
      )}
          ...
          {runs.map(run => (
            ...
            >
              <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center">
                ...
                {run.conclusion === 'cancelled' && <XCircle size={20} className="text-slate-500" />}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 truncate">
                  {run.head_commit?.message || 'No commit message'} · {timeAgo(run.created_at)}
                </p>
              </div>
